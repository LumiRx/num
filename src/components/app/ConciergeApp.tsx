...
import StayBookSheet from './StayBookSheet';
...
export default function ConciergeApp({ posterHeader = false, standalone = false }: { posterHeader?: boolean; standalone?: boolean }) {
  ...
  const sheetOpen = useApp((s) => s.calOpen || s.shareOpen || s.walletOpen || s.partyOpen || s.eventOpen || s.businessOpen || !!s.payOpen || s.passengerOpen || !!s.inviteOpen || !!s.tabOpen || s.errandsOpen || !!s.discoverOpen || s.placeOpen || s.flightWatchOpen || !!s.featureOpen || !!s.listingOpen || !!s.eventView || s.nightlifeOpen || !!s.stayBookOpen);
  ...
  const closeSheets = () => store.set({ calOpen: false, shareOpen: false, walletOpen: false, partyOpen: false, eventOpen: false, businessOpen: false, inviteOpen: null, payOpen: null, passengerOpen: false, tabOpen: null, errandsOpen: false, discoverOpen: null, placeOpen: false, flightWatchOpen: false, featureOpen: null, listingOpen: null, eventView: null, nightlifeOpen: false, stayBookOpen: null });

  const overlayOpen = useApp((s) => s.calOpen || s.shareOpen || s.walletOpen || s.partyOpen || s.eventOpen || s.businessOpen || !!s.payOpen || s.passengerOpen || !!s.inviteOpen || !!s.tabOpen || s.errandsOpen || !!s.discoverOpen || s.placeOpen || s.flightWatchOpen || !!s.featureOpen || !!s.listingOpen || !!s.eventView || s.nightlifeOpen || !!s.stayBookOpen || s.researchOpen || s.voice > 0);
  ...
  useEffect(() => {
    ...
    const onPop = () => {
      ...
      popped = true;
      store.set({ calOpen: false, shareOpen: false, walletOpen: false, partyOpen: false, eventOpen: false, businessOpen: false, inviteOpen: null, payOpen: null, passengerOpen: false, tabOpen: null, errandsOpen: false, discoverOpen: null, placeOpen: false, flightWatchOpen: false, featureOpen: null, listingOpen: null, eventView: null, nightlifeOpen: false, stayBookOpen: null });
  ...
  const onEscape = (e: KeyboardEvent) => {
    ...
    const s = store.get();
    if (s.calOpen || s.shareOpen || s.walletOpen || s.partyOpen || s.eventOpen || s.businessOpen || s.inviteOpen || s.payOpen || s.passengerOpen || s.tabOpen || s.errandsOpen || s.discoverOpen || s.placeOpen || s.flightWatchOpen || s.featureOpen || s.listingOpen || s.eventView || s.nightlifeOpen || !!s.stayBookOpen) closeSheets();
