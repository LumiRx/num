  ...
  return (
    <div ref={ref} role="dialog" aria-modal="true" className="glass-strong sheet-in" style={{ ...sheetBase, visibility: 'visible', transform: 'translateY(0)', maxHeight: 'min(92%, calc(100% - var(--safe-top, 0px)))', overflowY: 'auto' }}>
      ...
      <div style={{ padding: 16, display: 'grid', gap: 12 }}>
        <div>
          <div style={{ fontSize: 10, letterSpacing: '.14em', color: 'var(--color-accent)', fontWeight: 700 }}>{t('FLIGHT WATCH')}</div>
          <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 19, marginTop: 6 }}>{t('Give NUM the flight. It watches.')}</div>
          <div style={{ fontSize: 12, color: 'var(--ink-60)', marginTop: 4, lineHeight: 1.5 }}>{t('One ping when the gate, the time or the plan changes. Nothing otherwise.')}</div>
        </div>
        {flights.map((w) => (
        ...
        {!me && <div style={{ fontSize: 12, color: 'var(--ink-60)' }}>{t('Tell NUM who you are first, so it knows who to ping.')}</div>}
