...
`/api/health` returned 503 `down, failing 1` from 3 Sep to 18 Sep 03:27 UTC while
site, D1 writes, brain and attribution were green on every run. The one "failure"
was NUM's own `✅ Num is healthy again.` alert: triage correctly HELD it (not worth a
text), a held alert is untold by design, and `summary()` read untold-and-settled as
`blind` → `failures` on the DOWN list → DOWN alert → recovery → another held
"healthy again". Sixteen laps.

Fix (0.8.337): `failures.held()` stamps a held row `told_via = 'held:<judge>'`
(still `told = 0`, still open so the digest carries it); `summary()` skips held rows
when computing `blind`. An alert NOTHING carried is unchanged — still open, still
blind, and `alert_undelivered` (critical) still forces the verdict. Pinned in
`falsedown.test.mjs` and `failurecount.test.mjs`. The stuck production row was
stamped by hand (`told_via = 'held:backfill-2026-09-17'`) because the new code can
only stamp on the next hold, and no hold can happen while the verdict is pinned.

Watch for: `told_via` doubles as the held marker. `record()`'s reopen path resets
`told` but keeps `told_via`, so a reopened row that was once delivered reads
`told 0 / told_via sms` until the next `alert()` call stamps it one way or the other.
...
- ~~`/api/social/requests` drops the connection when a member id contains a quote character.~~ **Fixed 18 Sep, and it was never the quote.** Every query there binds its parameters, so `'`, `"`, `\`, a smart quote and `' OR 1=1--` all answer 200 with empty lists — verified against production. What actually threw was the last query, `num_event_guests JOIN num_events`: those tables belong to `worker/events.mjs` and are built by ITS `ensure()`, which a member who only opens the app has never triggered. So the inbox 500'd for every REAL member on a database where the events routes had not been used, and an unknown id returned before reaching that query — which is why the bug survived being looked at. `events.mjs` now exports `ensureEvents()` and `requests()` calls it, rather than a second copy of the schema drifting from the first. `worker/socialrequests.test.mjs` pins both halves (16 tests).
