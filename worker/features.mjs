...
export const FEATURES = Object.freeze([
  ...
  },
  {
    ...
    ready: (env) => has(env, 'GOOGLE_PLACES_API_KEY'),
    surface: 'invisible — it ranks everything else',
    code: ['worker/placeratings.mjs', 'ai/places.js'],
    sop: {
      on: 'Set GOOGLE_PLACES_API_KEY — the same key scripts/enrich_ratings.mjs has used since 11 Aug, now the only one. Enable the Places API (New) on that key and leave billing on. One search per ~1 km cell per category per 30 days; the cost is bounded by geography, not by traffic.',
      ...
      broken: 'THIS FEATURE READS "on" WHENEVER THE KEY IS SET, INCLUDING WHEN EVERY SEARCH IS REFUSED — it 429ed for nine hours on 18 Sep while this line said on, and then for three more days: 440 refusals on SerpAPI before it was moved to Google directly on 21 Sep. A refusal (429 spent plan, 401/403 bad key) now records a low-severity `ratings_refused` chore carrying the remedy: GET /api/admin/failures, or the chores count on /api/health. matched near zero is different and harmless — it means the directory has no rows where people are asking. Off entirely = the old ranking; picks get duller, nothing errors.',
    },
  },
  {
    id: 'flightwatch',
  ...
  // are not travel benefits under §17550.27, so a ceiling here is lawful.
  {
    ...
    sop: {
      on: 'Always on. `plans_max` meters how many run at once: 3 free, unlimited on NUM Plus. The first time a free member passes 3 they are given the plan anyway, once ever (num_usage_counters, period \'life\', key plans_max_gift).',
  ...
  },
  {
    ...
    entitlement: null,
    name: 'NUM Plus',
    does: 'The one paid plan — $4.99 a month or $49 a year, no ceilings, new features first. Never a travel deal: those are free to everyone, by law (§17550.27).',
    needs: ['STRIPE_SECRET_KEY'],
    ready: (env) => has(env, 'STRIPE_SECRET_KEY'),
