...
export const FEATURES = Object.freeze([
  ...
  },
  {
    ...
    needs: ['migration 0045', 'nothing else — a venue with no price list types the lines, and a venue that wants a total still types a total'],
    ready: (env) => !!env.DB,
  ...
  },
  {
    ...
    does: 'One bill becomes one real bill code per person, each paid straight to the venue, each sent to that person\'s NUM.',
    ...
    code: ['worker/billsplit.mjs', 'worker/billqr.mjs', 'src/components/app/BillSheet.tsx'],
    sop: {
  ...
  },
  {
    ...
    code: ['growth/pos/index.mjs', 'growth/pos/square.mjs', 'worker/billpay.mjs', 'worker/migrations/0039_wallets_and_pos.sql'],
    sop: {
      on: 'Run 0039. Set POS_TOKEN_KEY (any long random string — it encrypts merchant tokens at rest) plus the Square app id and secret on num-growth AND num-app, because the console connects the till and the Stripe webhook closes the check. A venue connects from Pay → Your till and picks a location.',
