 ...
 */
export function contextBlock({ now = new Date(), place = null, partners = [], widened = false, membership = null, guide = null, profile = {}, buzz = [], services = null, style = null, party = null, trip = null, air = false, acceptLang = null, showtimes = null, events = null, shown = null, entryDocs = null, essentials = null, holidays = null, passport = null, health = null, fares = null } = {}) {
  const lines = [];
  ...
  // ── WHAT THEY ARE ON, AND WHETHER TODAY IS THE DAY TO MENTION IT ────
  //
  // Placed AFTER the partner block on purpose: the answer comes first, and
  // the model reads this last. 152 members and 0 subscriptions on 16 Sep
  // 2026 — not because checkout was missing, but because nothing in this
  // prompt had ever heard of a plan. See worker/upgrade.mjs, which decides
  // whether an offer has been EARNED this turn and says no by default.
  ...
  if (partners.length) {
    ...
    lines.push(
      (ranked
        ...
        + 'The value in [square brackets] is the PARTNER ID. Copy it into each pick — it is what attaches the\n'
        + 'tappable link, the phone and the address to the card the guest sees. A pick without it is matched by name\n'
        + 'instead, and a name that does not match exactly is DROPPED before the guest ever sees it.\n'
        + 'THIS LIST IS A SHORTLIST TO CHOOSE FROM, NOT A LIST TO READ OUT. Offer the best, and keep the rest in reserve for when\n'
        + 'they ask again — they are here so that "show me others" has somewhere to go.\n'
        +
        partners
          .map(
            (b) =>
              `- [${b.id}] ${b.name}${b.name_local && b.name_local !== b.name ? ` (${b.name_local})` : ''} — ${b.category}${b.area ? `, ${b.area}` : ''}${b.km != null ? `, ${b.km < 1 ? Math.round(b.km * 1000) + ' m' : b.km + ' km'} away` : ''}${b.rating ? `, ${b.rating}★ (${b.reviews} reviews)` : ''}${b.phone ? `, ${b.phone}` : ''}${b.address ? `, ${b.address}` : ''}${b.website ? ', has a website' : ''}` +
          ...
          items: {
            ...
            properties: {
              ...
              why: { type: 'string', description:
                // Was "twelve words or fewer" until 15 Sep 2026. Twelve words is a
                // fragment, and users told Dre the suggestions read as thin. This is
                // the ONE place detail belongs — the prose reply stays short by
                // design, so if the reason to go is not here it is nowhere.
                'Why THIS one, in one full sentence of about 25 words. Name the thing that separates it from '
                + 'the others — the dish it is known for, the room, the crowd, the view, the price, how far it is. '
                + 'Concrete and specific: "the grilled branzino and a terrace that catches the sunset" beats '
                + '"great seafood with a nice atmosphere". Never invent a detail you were not given; if all you '
                + 'have is the category and the rating, say something true and plain instead.' },
