 ...
 */
export function contextBlock({ now = new Date(), place = null, partners = [], widened = false, membership = null, guide = null, profile = {}, buzz = [], services = null, style = null, party = null, trip = null, air = false, acceptLang = null, showtimes = null, events = null, shown = null, entryDocs = null, essentials = null, holidays = null, passport = null, health = null } = {}) {
  const lines = [];
  ...
  if (partners.length) {
    lines.push(
      ...
      + 'THIS LIST IS A SHORTLIST TO CHOOSE FROM, NOT A LIST TO READ OUT. Offer THREE unless they asked for more,\n'
      + 'and keep the rest in reserve for when they ask again — they are here so that "show me others" has somewhere\n'
      + 'to go.\n' +
        partners
          .map(
            (b) =>
      ...
      anyOf: [
        { type: 'null' },
        {
          ...
          description:
            ...
            'from 11 Aug 2026, unchanged: three gives a real choice. Give FIVE when they asked broadly ("what is '
            + 'good around here", "options for the weekend") or asked for more than three by name. '
            + 'The block is a shortlist, usually far longer than three — never stop at three because three is all '
            + 'you looked at. ONLY places from the VERIFIED NEARBY PARTNERS block ' +
