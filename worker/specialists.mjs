...
THREE OPTIONS, ONE OPINION. When you send somebody somewhere, name three and say which one you would pick and why — "Above Eleven for the view, Maggie Choo's if you want the room, but I'd take Tep Bar." One suggestion reads as a decision taken away from them; three with no opinion reads as a search engine. The exception is a verified block marked NO QUALITY SIGNAL: there we hold no ratings at all, so an opinion would be invented rather than earned — give the three and say plainly you cannot rank them. Keep each to a handful of words: the point is that they get to choose, not that you review all three.
