...
// Both sides need it. The app draws a member's code in the browser; the
// business console is server-rendered HTML from the Worker and has to draw a
// venue's code with no browser anywhere in the picture. Two copies of a QR
// encoder is two things that can disagree about a mask and produce a code that
// scans on one screen and not the other. So the implementation lives here, in
// plain JavaScript that both the Worker and Vite can import, and
// src/lib/qr.ts is a typed re-export of it. There is exactly one encoder.
...
// ── matrix ───────────────────────────────────────────────────────────────────
// A grid is { size, m, fn }; `fn` marks function modules.

const at = (g, r, c) => g.m[r * g.size + c];
const set = (g, r, c, v, isFn = false) => {
  g.m[r * g.size + c] = v;
...
function penalty(g) {
  ...
  const match = (get, start, pat) => pat.every((p, i) => get(start + i) === p);
  for (let r = 0; r < n; r++) {
...
export function qrMatrix(text) {
  ...
  const base = { size, m: new Int8Array(size * size), fn: new Uint8Array(size * size) };
  ...
  for (let mask = 0; mask < 8; mask++) {
    const g = { size, m: Int8Array.from(base.m), fn: Uint8Array.from(base.fn) };
