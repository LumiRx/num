...
const OUT = join(ROOT, 'src/i18n/catalog.ts');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (process.argv.includes('--check')) {
    ...
    if (!have || have.hash !== fresh.hash) {
      console.error(`src/i18n/catalog.ts is stale (${have?.strings?.length ?? 0} → ${fresh.strings.length} strings). Run: node scripts/i18n-catalog.mjs`);
      process.exit(1);
    }
    console.log(`catalog ok: ${fresh.strings.length} strings`);
  } else {
    mkdirSync(join(ROOT, 'src/i18n'), { recursive: true });
    writeFileSync(OUT, `// Generated by scripts/i18n-catalog.mjs — do not edit. ${fresh.strings.length} strings.\nexport const CATALOG = ${JSON.stringify(fresh, null, 1)} as const;\n`);
    console.log(`wrote ${fresh.strings.length} strings → src/i18n/catalog.ts`);
  }
}
